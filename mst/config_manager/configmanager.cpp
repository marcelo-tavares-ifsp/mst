...
{
    string out_file = PathManager::get_rclua_config();
    ...
    if (version[0] == 3)
    {
        ...
        rclua_pattern.open(PathManager::get_rclua_template(), ios::in);
    }
    else
    {
        qDebug(config_manager_category) << "Using rc.lua.template for Awesome 4";
        rclua_pattern.open(PathManager::get_rclua4_template(), ios::in);
    }
...
{
    string out_file = PathManager::get_xorg_config();
...
{
    string out_file = PathManager::get_bashrc_config();
    string in_file = PathManager::get_bashrc_config_template();
...
{
    string out_file = PathManager::get_xinitrc_config();
    string in_file = PathManager::get_xinitrc_config_template();
    ...
    out_file = PathManager::get_xmst_config();
    in_file  = PathManager::get_xmst_config_template();
...
void ConfigManager::make_sudoers()
{
    ...
    const string out_file = PathManager::get_sudoers_config();
    const string in_file = PathManager::get_sudoers_config_template();
...
{
    const string out_file = PathManager::get_lightdm_mst_config();
    const string in_file = PathManager::get_lightdm_mst_config_template();
    ofstream out(out_file);
    ifstream in(in_file);
...
{
    const string out_file = PathManager::get_getty_service_config();
    const string in_file = PathManager::get_getty_service_config_template();
    const string user = PathManager::get_mst_user();
    ifstream in(in_file);
    ofstream out(out_file);
...
{
    const string out_file = PathManager::get_udev_rules_config();
    ofstream out(out_file);
...
{
    const string out_file = PathManager::get_systemd_udev_config();
    const string in_file = PathManager::get_systemd_udev_config_template();
    ofstream out(out_file);
    ifstream in(in_file);
...
{
    const string out_file = PathManager::get_vgl_config();
    const string in_file = PathManager::get_vgl_config_template();
    const string user = PathManager::get_mst_user();
    ofstream out(out_file);
    ifstream in(in_file);
